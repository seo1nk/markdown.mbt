...
export class LiteralEditor {
  ...
  private readonly render: LiteralRenderer;
  private current: string;

  constructor(container: HTMLElement, render: LiteralRenderer, initialSource = "") {
 ...
 */
export function patchTopLevelChildren(container: HTMLElement, newHtml: string): PatchStats {
  const template = container.ownerDocument.createElement("template");
  template.innerHTML = newHtml;
  const next = Array.from(template.content.childNodes);
  const prev = Array.from(container.childNodes);
  const stats: PatchStats = { ...ZERO_STATS };
...
const POS_ATTRS = ["data-src-start", "data-src-end"] as const;

function isShiftOnly(oldEl: HTMLElement, newEl: HTMLElement): boolean {
  if (oldEl.tagName !== newEl.tagName) return false;
  if (oldEl.innerHTML !== newEl.innerHTML) return false;
  ...
  for (let i = 0; i < oldAttrs.length; i++) {
    ...
    if (attr.value === newVal) continue;
    if ((POS_ATTRS as readonly string[]).includes(attr.name)) continue;
    return false;
  }
  return true;
}

function copyPositionAttrs(from: HTMLElement, to: HTMLElement): void {
